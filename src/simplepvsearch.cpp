...
int SimplePVSearch::rootSearch(Board &board, SearchInfo &si, PvLine &pv) {
    ...
    while (rootMoves.hasNext() && !stop(si)) {
        ...
        SearchInfo newSi(true, move, -beta, -alpha, newDepth, si.ply + 1, PV_NODE, nullptr);
        ...
        if (score >= si.beta) {
            ...
            if (okToPrune) {
                const TranspositionTable::NodeFlag flag = currentScore != -maxScore && !isLazyEval ?
                                                          TranspositionTable::NM_LOWER_EVAL
                                                                                                   : TranspositionTable::NM_LOWER;
...
const bool SimplePVSearch::stop(SearchInfo &info) {
    ...
    return info.splitPoint != nullptr && info.splitPoint->shouldStop;
}
