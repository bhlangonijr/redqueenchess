...
int SimplePVSearch::pvSearch(Board& board, SearchInfo& si, int alpha, int beta,	int depth, int ply, PvLine* pv) {
	...
	while (true) {
		...
		MoveIterator::Move& move = selectMove<false>(board, moves, hashMove, isKingAttacked, ply, depth, false);
		if (move.none()) {
			break;
		}
		MoveBackup backup;
...
int SimplePVSearch::qSearch(Board& board, SearchInfo& si,
	...
	while (true)  {
		...
		MoveIterator::Move& move = selectMove<true>(board, moves, hashMove, isKingAttacked, ply, depth, searchForChecks);
		if (move.none()) {
			break;
		}
		MoveBackup backup;
...
long SimplePVSearch::perft(Board& board, int depth, int ply) {
	...
	while (true)  {
		...
		MoveIterator::Move& move = selectMove<false>(board, moves, emptyMove, isKingAttacked, ply, depth, false);
		if (moves.end()) {
			break;
		}
		MoveBackup backup;
