...
SearchAgent::SearchAgent() : searchMode(SEARCH_TIME), searchInProgress(false), requestStop(false), quit(false),
                             ...
                             blackIncrement(0), depth(defaultDepth), movesToGo(0), searchNodes(0), searchMoves("none"),
                             moveTime(0), ponder(false),
...
const bool SearchAgent::spawnThreads(Board &board, void *data, const int currentThreadId,
                                     MoveIterator *moves, MoveIterator::Move *move, MoveIterator::Move *hashMove,
                                     int *bestScore,
...
void SearchAgent::initThreads() {
    ...
    for (int i = 1; i < threadPoolSize; i++) {
        rCode = pthread_create(&(threadPool[i].executor), NULL, workerThreadRun, this);
        if (rCode) {
