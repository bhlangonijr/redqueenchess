...
public:
    ...
    SimplePVSearch(int *_history) : depthToSearch(maxSearchDepth), updateUci(true), timeToSearch(0), startTime(0),
                                    searchFixedDepth(false),
                                    infinite(false), nodes(0), maxPlySearched(0), nodesToGo(defaultNodesToGo),
                                    history(_history) {
        initMutex();
    }
...
inline void SimplePVSearch::uciOutput(PvLine &pv, const int score, const int64_t totalTime,
                                      const int hashFull, const int depth, const int selDepth, const int alpha,
                                      const int beta) {
